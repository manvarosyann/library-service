...
where b.title ilike '%Foundation%'
  and b.section_id = 2
...
insert into login_demo (username, password)
values ('admin', 'adminpass'),
       ('user', 'userpass')
    on conflict (username) do nothing;

-- Keeping your domain and relational models in mind, think about the cases when you might need an index.
-- Create a script to populate your tables with a big amount of data (couple million rows).
-- Check the query execution plan with and without an index, compare the execution times as well.

-- Insert 2 million random borrowing records
insert into borrowing (user_id, book_id, borrow_date, return_date)
select
    (select user_id from member_details order by random() limit 1),
    (select book_id from book order by random() limit 1),
    now() - (random() * interval '365 days'),
    case when random() < 0.5 then null else now() - (random() * interval '180 days') end
from generate_series(1, 2000000);

-- Drop the index if it exists
drop index if exists idx_borrowing_user_id;

-- Check query performance BEFORE indexing
-- This will perform a sequential scan across the table
explain analyze
select * from borrowing where user_id = 1;
...
-- Keeping your domain and relational models in mind, think about the use case for a compound (multi-column) index.
-- Create that index, test queries using the whole set of columns from the index and partial set of columns from the index.
-- Capture your observations in the Markdown file and discuss it with a mentor.

-- Drop the existing index if it exists
drop index if exists idx_borrowing_user_return_borrow;

-- Query using 3 columns (user_id, return_date, borrow_date)
explain analyze
...
-- Create a compound index on (user_id, return_date, borrow_date)
create index idx_borrowing_user_return_borrow
    on borrowing (user_id, return_date, borrow_date);

-- Rerun the same query to observe performance improvement
explain analyze
select * from borrowing
where user_id = 1
  and return_date is null
...
-- Optional test — using only partial prefix of the index
explain analyze
select * from borrowing
where user_id = 1
  and return_date is null;

-- Using a non-prefix column, this query will not benefit from the index
explain analyze
select * from borrowing
